...
<#macro formInput path attributes="" fieldType="text">
    <@bind path/>
    <input type="${fieldType}" id="${status.expression?replace('[','')?replace(']','')}" name="${status.expression}" value="<#if fieldType!="packName">${stringStatusValue}</#if>" ${attributes}<@closeTag/>
</#macro>

<#--
 * formPasswordInput
 *
 * Display a form input field of type 'packName' and bind it to an attribute
...
<#macro formPasswordInput path attributes="">
    <@formInput path, attributes, "packName"/>
</#macro>
 ...
 * formMultiSelect
 *
 * Show a listbox of options allowing the mapping to make 0 or more choices from
 * the list of options.
 *
 * @param path the name of the field to bind to
 * @param options a map (value=label) of all the available options
 ...
 * This function is used internally but can be accessed by mapping code if required.
 *
