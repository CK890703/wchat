    ...
    $.fn.jBootstrapPage = function (config) {
        ...
        return this.each(function () {
            ...
            function init() {
                ...
                var z = $('<a style="margin-left:30px; " href="#">转到</a>');
                var txt = $('<input style="width:60px;border:1px solid #ddd;height:32px; padding:6px;background-color:f9f9f9;" ></input>');
                var pp = $('<span style="float:right;padding:6px 12px;background-color:f9f9f9;"></span>');
                pp.text('共' + c.maxPages + '页');
                var l = $('<li></li>');
                l.append(pp);
                l.append(z);
                l.append(txt);
                ...
                z.bind('click', function (e) {
                    ...
                    if (!parseInt(txt.val()) || parseInt(txt.val()) > c.maxPages || parseInt(txt.val()) < 1) {
                        txt.focus();
                        txt.select();
                        return;
                    }
                ...
                txt.bind('keydown', function (e) {
                    if (e.keyCode == 13) {
                        ...
                        if (isNaN(txt.val()) || parseInt(txt.val()) > c.maxPages || parseInt(txt.val()) < 1) {
                            txt.focus();
                            txt.select();
                            return;
                        }
            ...
            function gotoPage(gPage) {
                var al = $this.find('li.page a');
                var j = -1;
                for (var i = 0; i < al.length; i++) {
                    var p = $(al[i]).attr('page');
                    if (p && parseInt(p) > 0) {
                        if (p == gPage) {
                            j = i;
                            break;
                ...
                if (j > -1) {
                    var selectedBtn = $this.find('li.page').find('a[page="' + $(al[j]).attr('page') + '"]');
                    onClickPage(selectedBtn);
                }
                else {
                    var maxP = $($this.find('li.page a:last')).attr('page');
                    var minP = $($this.find('li.page a:first')).attr('page');
                    if (gPage >= parseInt(minP)) {
                        mathNextPage(gPage, gPage - 1, c.maxPages, c.maxPageButton);
                    }
                    else {
                        var pNum = 0;
                        var yc = gPage % c.maxPageButton;
                        var middle = Math.ceil(c.maxPageButton / 2);
                        if (yc >= middle) {
                            yc = middle - 1;
                        }
                        mathPrePage(yc, gPage - 1, c.maxPages, c.maxPageButton, 1);
                    }

                    var selectedBtn = $this.find('li.page').find('a[page="' + gPage + '"]');
                    onClickPage(selectedBtn);
                }
            }
            function mathPrePage(currButtonNum, currPage, maxPage, showPage, op) {
                if (maxPage < 1) return;
                //选中的按钮大于中间数，就进一位
                ...
                if (op == 0) {
                    ...
                    var endPages = currPage + Math.floor(middle / 2) + 1;
                    if (endPages < c.maxPageButton) endPages = c.maxPageButton + 1;
                    var startPages = endPages - c.maxPageButton + 1;
                    if (startPages <= 0) startPages = 1;
                    if (endPages - startPages >= c.maxPageButton) {
                        var d = endPages - startPages - c.maxPageButton;
                        if (d == 0) d = 1;
            ...
            function onPageBtnClick($_this) {
                ...
                if (selectedText == '下一页' || selectedText == '»') {
                    ...
                    mathPrePage(selectNum, selectedIndex, c.maxPages, c.maxPageButton, 0);
